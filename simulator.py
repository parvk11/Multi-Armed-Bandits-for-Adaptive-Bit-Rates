...
def main(config_file: str, student_algo, verbose: bool, print_output=True) -> Tuple[float, float, float, float, List]:
	...
	assert os.path.exists(f'./student/{student_algo}.py'),\
		f'Could not find student algorithm ./student/{student_algo}.py!'
	student = importlib.import_module(f'student.{student_algo}')
